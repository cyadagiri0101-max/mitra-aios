...
process.env.DB_USERNAME = process.env.DB_USERNAME ?? 'mitra_admin';
// Must match the password configured for the mitra_admin role in this environment.
process.env.DB_NAME = process.env.E2E_DB_NAME ?? 'mitra_v2_test';
process.env.DB_SYNC = 'false';
process.env.DB_LOGGING = 'false';
process.env.DB_SSL = 'false';
// DB_PASSWORD is intentionally NOT set here — it is loaded from the
// developer's .env (dotenv does not overwrite vars already set, so every
// other key above takes precedence, but DB_PASSWORD falls through to
// whatever the mitra_admin role's real password is in this environment).
