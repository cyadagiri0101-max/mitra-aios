...
import { CreateCPSReviewDto } from '../dto/cps.dto';
...
@Controller('cps/reviews')
export class CPSReviewController {
  constructor(private readonly service: CpsReviewService) {}
  ...
  @Post()
  async create(@Body() dto: CreateCPSReviewDto, @CurrentUser() user: AuthUser) {
    return this.service.create(dto as unknown as Record<string, unknown>, user.id, user.tenantId ?? undefined);
  }

  @UseGuards(RolesGuard)
  @Roles('ADMIN', 'MANAGEMENT', 'SALES', 'DESIGN', 'PLANNING', 'PRODUCTION', 'QUALITY')
  @Permissions('trial:approve')
  @Patch(':id')
  async update(@Param('id', ParseUUIDPipe) id: string, @Body() dto: Partial<CreateCPSReviewDto>, @CurrentUser() user: AuthUser) {
