...
import { CreateMoldStructureDto, UpdateMoldStructureDto } from '../dto/mold.dto';
...
@Controller('mold/structures')
export class MoldStructureController {
  constructor(private readonly service: MoldStructureService) {}
  ...
  @Post()
  async create(
    @Body() dto: CreateMoldStructureDto,
    @CurrentUser() user: AuthUser,
  ) {
  ...
  async update(
    @Param('id', ParseUUIDPipe) id: string,
    @Body() dto: Partial<CreateMoldStructureDto>,
    @CurrentUser() user: AuthUser,
  ) {
