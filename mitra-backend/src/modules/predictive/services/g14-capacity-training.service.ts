...
import {
  ...
  G14CapacityEvaluationMetrics,
} from '../dto/g14-capacity.dto';
import { G14_CURRENT_CAPACITY_VERSION } from './g14-capacity-feature.service';

export const G14_DEFAULT_CAPACITY_MODEL_VERSION = 'G14_CAPACITY_RIDGE_V1';

@Injectable()
export class G14CapacityTrainingService {
  private readonly logger = new Logger(G14CapacityTrainingService.name);
  ...
  protected requireTenant(tenantId?: string | null): string {
    if (!tenantId) {
      throw new ForbiddenException('Tenant context required for capacity model training');
    }
    return tenantId;
  }
  ...
  /**
   * Trains a governed G14 predictive capacity deficit model.
   */
  async trainCapacityModel(
    dto: TrainCapacityModelDto,
    tenantId: string,
  ): Promise<G14CapacityModelArtifact> {
    ...
    if (featureVersion !== G14_CURRENT_CAPACITY_VERSION) {
      throw new BadRequestException(
        `Cannot train capacity model for unsupported feature version: ${featureVersion}`,
      );
    }
    ...
    const validSnapshots = snapshots.filter(
      (s) =>
        ...
        typeof s.groundTruthTarget.capacityDeficitHours === 'number' &&
        Array.isArray(s.featureVector?.denseVector) &&
        s.featureVector.denseVector.length === 15,
    );

    if (validSnapshots.length < minSamples) {
      throw new BadRequestException(
        `Insufficient historical capacity snapshots (${validSnapshots.length} available, ${minSamples} required).`,
      );
    }
    ...
    const y_train = trainSet.map((s) => s.groundTruthTarget!.capacityDeficitHours);

    // 3. Train Ridge Regression
    const { weights, intercept } = this.solveRidgeRegression(X_train, y_train, alpha);

    // 4. Compute Baseline Naive MAE
    const naiveMeanDeficit = y_train.reduce((a, b) => a + b, 0) / y_train.length;
    ...
    let sumTotalVar = 0;
    let correctOverloadClassification = 0;
    let brierScoreSum = 0;

    for (const sample of evalSet) {
      const x = sample.featureVector.denseVector;
      const y = sample.groundTruthTarget!.capacityDeficitHours;
      const y_isOverloaded = sample.groundTruthTarget!.isOverloaded ? 1 : 0;
      ...
      sumAbsError += absErr;
      sumSqError += sqErr;
      sumNaiveAbsError += Math.abs(y - naiveMeanDeficit);
      sumTotalVar += (y - naiveMeanDeficit) ** 2;

      const p_overload = 1 / (1 + Math.exp(-(y_pred / 10)));
      const predIsOverload = p_overload >= 0.5 ? 1 : 0;
      if (predIsOverload === y_isOverloaded) {
        correctOverloadClassification++;
      }
      brierScoreSum += (p_overload - y_isOverloaded) ** 2;
    }
    ...
    const accuracy = nEval > 0 ? correctOverloadClassification / nEval : 1.0;
    const brierScore = nEval > 0 ? brierScoreSum / nEval : 0;
    const residualStdDev = rmse > 0 ? rmse : 1.0;

    const evaluationMetrics: G14CapacityEvaluationMetrics = {
    ...
    const modelVersion = `${G14_DEFAULT_CAPACITY_MODEL_VERSION}_${Date.now()}`;

    // 7. Deactivate older active capacity models
    ...
    const artifact = await this.modelRepository.save({
      ...
      modelVersion,
      featureVersion,
      modelType: dto.modelType || 'RIDGE_CAPACITY_DEFICIT_REGRESSION',
      weights,
    ...
    await this.auditService.log({
      ...
      entityType: 'G14CapacityModelArtifact',
  ...
  /**
   * Retrieves the active trained capacity model artifact for a tenant.
   */
  async getActiveModel(tenantId: string): Promise<G14CapacityModelArtifact | null> {
    const scopeTenant = this.requireTenant(tenantId);
