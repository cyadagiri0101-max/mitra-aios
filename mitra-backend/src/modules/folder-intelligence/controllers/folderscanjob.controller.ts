...
import { CreateFolderScanJobDto } from '../dto/folder.dto';
import { JwtAuthGuard } from '@common/guards/jwt-auth.guard';
import { RolesGuard } from '@common/guards/roles.guard';
import { Roles } from '@common/decorators/roles.decorator';
import { PaginationDto } from '@common/dto/pagination.dto';

@ApiTags('folder-intelligence')
@ApiBearerAuth()
@UseGuards(JwtAuthGuard)
@Controller('folder-intelligence/jobs')
export class FolderScanJobController {
  constructor(private readonly service: FolderScanJobService) {}
  ...
  @Post()
  async create(@Body() dto: CreateFolderScanJobDto, @CurrentUser() user: AuthUser) {
  ...
  @Roles('ADMIN', 'MANAGEMENT', 'SALES', 'DESIGN', 'PLANNING', 'PRODUCTION', 'QUALITY')
  @Patch(':id')
  async update(@Param('id', ParseUUIDPipe) id: string, @Body() dto: Partial<CreateFolderScanJobDto>, @CurrentUser() user: AuthUser) {
