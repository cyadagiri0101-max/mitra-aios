...
class OllamaProvider(LLMProvider):
    def __init__(self, config: ProviderConfig | None = None) -> None:
        self.logger = get_logger("aios.llm.ollama_provider")
    ...
    def initialize(self) -> OllamaProvider:
        self._initialized = True
        self.logger.info("OllamaProvider initialized (model=%s, base=%s)", self._config.model, self._config.api_base)
        return self
    ...
    def embed(self, request: EmbeddingRequest) -> EmbeddingResponse:
        self._require_initialized()
        raise NotImplementedError("Ollama embedding transport not implemented")
    ...
    def validate(self) -> ValidationResult:
        ...
        if not self._config.api_base:
            result.errors.append("Ollama API base URL not configured")
            result.is_valid = False
        if not self._config.model:
            result.errors.append("Ollama model not configured")
            result.is_valid = False
        return result

    def reload(self) -> OllamaProvider:
        self.logger.info("Reloading OllamaProvider")
        with self._lock:
            self._requests = 0
            self._successes = 0
            self._failures = 0
    ...
    def shutdown(self) -> None:
        self.logger.info("Shutting down OllamaProvider")
        self._initialized = False

    def _require_initialized(self) -> None:
        from aios.core.exceptions import LLMProviderError
        if not self._initialized:
            raise LLMProviderError("OllamaProvider has not been initialized", provider=self.name)
