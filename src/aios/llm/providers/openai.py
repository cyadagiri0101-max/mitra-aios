...
class OpenAIProvider(LLMProvider):
    def __init__(self, config: ProviderConfig | None = None) -> None:
        self.logger = get_logger("aios.llm.openai_provider")
        self._config = config or ProviderConfig(name="openai", model="gpt-4")
    ...
    def initialize(self) -> OpenAIProvider:
        self._initialized = True
        self.logger.info("OpenAIProvider initialized (model=%s)", self._config.model)
        return self
    ...
    def stream(self, request: LLMRequest) -> list[str]:
        self._require_initialized()
        self._validate_config()
        raise NotImplementedError("OpenAI streaming transport not implemented")

    def embed(self, request: EmbeddingRequest) -> EmbeddingResponse:
        self._require_initialized()
        self._validate_config()
        raise NotImplementedError("OpenAI embedding transport not implemented")
    ...
    def reload(self) -> OpenAIProvider:
        self.logger.info("Reloading OpenAIProvider")
        with self._lock:
            self._requests = 0
            self._successes = 0
            self._failures = 0
    ...
    def shutdown(self) -> None:
        self.logger.info("Shutting down OpenAIProvider")
        self._initialized = False

    def _validate_config(self) -> None:
        from aios.core.exceptions import LLMProviderError
        if not self._config.api_key:
            raise LLMProviderError("OpenAI API key not configured", provider=self.name)

    def _require_initialized(self) -> None:
        from aios.core.exceptions import LLMProviderError
        if not self._initialized:
            raise LLMProviderError("OpenAIProvider has not been initialized", provider=self.name)
