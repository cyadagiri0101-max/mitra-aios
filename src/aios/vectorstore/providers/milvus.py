...
class MilvusVectorStore(VectorStoreProvider):
    ...
    def __init__(self, config: VectorStoreConfig | None = None) -> None:
        self.logger = get_logger("aios.vectorstore.milvus")
    ...
    def initialize(self) -> MilvusVectorStore:
        """Initialize the provider."""
        self._initialized = True
        self.logger.info("MilvusVectorStore initialized (dimensions=%d)", self._config.dimensions)
        return self

    def upsert(self, records: list[VectorRecord]) -> int:
        """Insert or update vector records."""
        self._require_initialized()
        raise NotImplementedError("Milvus requires the pymilvus library")
    ...
    def validate(self) -> VectorStoreValidationResult:
        ...
        result.warnings.append("Milvus requires the pymilvus library")
        return result

    def reload(self) -> MilvusVectorStore:
        """Reload provider configuration."""
        self.logger.info("Reloading MilvusVectorStore")
        return self

    def shutdown(self) -> None:
        """Shutdown the provider."""
        self.logger.info("Shutting down MilvusVectorStore")
        self._initialized = False

    def _require_initialized(self) -> None:
        """Check if provider is initialized."""
        if not self._initialized:
            raise VectorStoreError(f"MilvusVectorStore '{self.name}' has not been initialized", provider=self.name)
