...
class WeaviateVectorStore(VectorStoreProvider):
    ...
    def __init__(self, config: VectorStoreConfig | None = None) -> None:
        self.logger = get_logger("aios.vectorstore.weaviate")
    ...
    def initialize(self) -> WeaviateVectorStore:
        """Initialize the provider."""
        self._initialized = True
        self.logger.info("WeaviateVectorStore initialized (dimensions=%d)", self._config.dimensions)
        return self

    def upsert(self, records: list[VectorRecord]) -> int:
        """Insert or update vector records."""
        self._require_initialized()
        raise NotImplementedError("Weaviate requires the weaviate-client library")
    ...
    def validate(self) -> VectorStoreValidationResult:
        ...
        result.warnings.append("Weaviate requires the weaviate-client library")
        return result

    def reload(self) -> WeaviateVectorStore:
        """Reload provider configuration."""
        self.logger.info("Reloading WeaviateVectorStore")
        return self

    def shutdown(self) -> None:
        """Shutdown the provider."""
        self.logger.info("Shutting down WeaviateVectorStore")
        self._initialized = False

    def _require_initialized(self) -> None:
        """Check if provider is initialized."""
        if not self._initialized:
            raise VectorStoreError(f"WeaviateVectorStore '{self.name}' has not been initialized", provider=self.name)
