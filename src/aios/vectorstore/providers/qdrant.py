...
class QdrantVectorStore(VectorStoreProvider):
    ...
    def __init__(self, config: VectorStoreConfig | None = None) -> None:
        self.logger = get_logger("aios.vectorstore.qdrant")
    ...
    def initialize(self) -> QdrantVectorStore:
        ...
        self.logger.info("QdrantVectorStore initialized (dimensions=%d)", self._config.dimensions)
        return self

    def upsert(self, records: list[VectorRecord]) -> int:
        """Insert or update vector records."""
        self._require_initialized()
        raise NotImplementedError("Qdrant requires the qdrant-client library")
    ...
    def validate(self) -> VectorStoreValidationResult:
        ...
        result.warnings.append("Qdrant requires the qdrant-client library")
        return result

    def reload(self) -> QdrantVectorStore:
        """Reload provider configuration."""
        self.logger.info("Reloading QdrantVectorStore")
        return self

    def shutdown(self) -> None:
        """Shutdown the provider."""
        self.logger.info("Shutting down QdrantVectorStore")
        self._initialized = False

    def _require_initialized(self) -> None:
        """Check if provider is initialized."""
        if not self._initialized:
            raise VectorStoreError(f"QdrantVectorStore '{self.name}' has not been initialized", provider=self.name)
