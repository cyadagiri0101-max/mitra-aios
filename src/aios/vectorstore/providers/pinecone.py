...
class PineconeVectorStore(VectorStoreProvider):
    ...
    def __init__(self, config: VectorStoreConfig | None = None) -> None:
        self.logger = get_logger("aios.vectorstore.pinecone")
    ...
    def initialize(self) -> PineconeVectorStore:
        ...
        self.logger.info("PineconeVectorStore initialized (dimensions=%d)", self._config.dimensions)
        return self

    def upsert(self, records: list[VectorRecord]) -> int:
        """Insert or update vector records."""
        self._require_initialized()
        raise NotImplementedError("Pinecone requires the pinecone-client library")
    ...
    def validate(self) -> VectorStoreValidationResult:
        ...
        result.warnings.append("Pinecone requires the pinecone-client library")
        return result

    def reload(self) -> PineconeVectorStore:
        """Reload provider configuration."""
        self.logger.info("Reloading PineconeVectorStore")
        return self

    def shutdown(self) -> None:
        """Shutdown the provider."""
        self.logger.info("Shutting down PineconeVectorStore")
        self._initialized = False

    def _require_initialized(self) -> None:
        """Check if provider is initialized."""
        if not self._initialized:
            raise VectorStoreError(f"PineconeVectorStore '{self.name}' has not been initialized", provider=self.name)
