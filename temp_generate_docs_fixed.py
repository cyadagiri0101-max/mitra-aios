            ...
            for target in node.targets:
                if isinstance(target, ast.Name) and target.id == 'sql':
                    ...
                    if sql_text is not None:
                        sql_assigns.append((node.lineno, sql_text.strip()))

    for lineno, sql_text in sql_assigns:
        purpose = 'sql assignment'
        for j in range(lineno - 1, min(lineno + 8, len(lines))):
            if 'add_evidence(' in lines[j]:
                match = re.search(r'"([^"]+)"\s*,\s*"([^"]+)"', lines[j])
                if match:
                    purpose = match.group(2)
                    break
            if j + 1 < len(lines) and 'add_evidence(' in lines[j] + lines[j + 1]:
                match = re.search(r'"([^"]+)"\s*,\s*"([^"]+)"', lines[j] + lines[j + 1])
                if match:
                    purpose = match.group(2)
                    break
        queries.append({'line': lineno, 'sql': sql_text, 'purpose': purpose})
